...
with open(DATA_PATH + 'cascade_train.txt', 'r') as f:
    for line in f:
        profile = line.split('\t')
        labels_train[profile[0]] = map(int, profile[-1].split())
...
with open(DATA_PATH + 'cascade_test.txt', 'r') as f:
    for line in f:
        profile = line.split('\t')
        labels_test[profile[0]] = map(int, profile[-1].split())
...
for label in labels_train.values():
    for i in label:
        original_ids.add(i)

for label in labels_test.values():
    for i in label:
        original_ids.add(i)
...
np.random.seed(13)
with open(DATA_PATH + 'deepwalk.txt', 'r') as f:
    line = f.readline()
